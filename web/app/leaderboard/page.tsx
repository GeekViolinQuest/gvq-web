...
export default function LeaderboardPage() {
  const [tab, setTab] = useState<"level" | "cristais">("level");
  const [q, setQ] = useState("");
  const [loading, setLoading] = useState(true);
  const [data, setData] = useState<LeaderRes | null>(null);
  const [error, setError] = useState<string | null>(null);

  async function load(t: "level" | "cristais") {
    setLoading(true);
    setError(null);
    try {
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((it) => it.displayName.toLowerCase().includes(query));
  }, [data, q]);
